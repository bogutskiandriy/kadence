...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'sprintit-agent-'));
...
describe('.sprintit/README.md', () => {
  ...
  it('contains the commands and the contract description', () => {
    runInit(dir);
    const text = readFileSync(join(dir, '.sprintit', 'README.md'), 'utf8');
    expect(text).toContain('sprintit task list --json');
    expect(text).toContain('sprintit/v1');
    expect(text).toContain('SPRINTIT_SOURCE');
  });

  it('is not overwritten when the user has edited it', () => {
    runInit(dir);
    writeFileSync(join(dir, '.sprintit', 'README.md'), 'my own rules');
    runInit(dir);
    expect(readFileSync(join(dir, '.sprintit', 'README.md'), 'utf8')).toBe('my own rules');
  });
});
...
describe('AGENTS.md', () => {
  it('is created when absent', () => {
    ...
    expect(readFileSync(agentsPath(), 'utf8')).toContain('sprintit');
  });
  ...
  it('is extended while human-written text is untouched', () => {
    ...
    expect(text).toContain('sprintit');
  });
  ...
  it('the section is not duplicated on a repeat init', () => {
    ...
    runInit(dir);
    const text = readFileSync(agentsPath(), 'utf8');
    expect(text.split('<!-- sprintit:begin -->').length - 1).toBe(1);
  });
  ...
  it('updates the section content without touching surrounding text', () => {
    ...
    expect(after.split('<!-- sprintit:begin -->').length - 1).toBe(1);
  });
});
