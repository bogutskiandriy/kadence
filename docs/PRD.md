# FlowIt — Product Requirements Document
...
**Непрямий сигнал, який уже є:** поява конвенцій `AGENTS.md` / `CLAUDE.md` та їх швидке поширення показує, що індустрія вже кладе контекст для агентів у репозиторій. FlowIt — логічне продовження цього тренду на рівень задач. 🔶 **Assumption:** що цей тренд пошириться з «інструкцій для агента» на «стан проєкту».
...
|---|---|---|
| **Primary** | Мала tech-heavy команда (2–10) | Точка входу adoption: ставить `npx flowit` без закупівельного циклу |
...
- **Поведінка:** не встановлюватиме CLI сама 🔶 **Assumption** — ключове обмеження: **без візуального дашборду вона не стане активним користувачем**. Це прямо обґрунтовує необхідність `flowit serve` у v2 (див. розділ 8).
...
- **Ціна вибору — ризик для adoption.** Багато компаній мають OSS-політики, що автоматично блокують AGPL-залежності, навіть коли copyleft юридично не спрацьовує: FlowIt лише запускається як CLI, не лінкується в продукт користувача, тому код проєкту не стає похідним твором. Сканери ліцензій цієї різниці не роблять.
- **Мітигація:** окремий `LICENSING.md` у корені репо, який прямим текстом пояснює межі copyleft для випадку «використовую FlowIt як інструмент». Написати його до публічного релізу, не після першої скарги.

### 4.2. Конкурентний ландшафт

> **Оновлено 2026-09-02 після конкурентного дослідження.** Попередня версія цього розділу порівнювала FlowIt лише з Jira, Linear і GitHub Issues — інструментами іншого класу — і повністю пропускала прямих конкурентів. Це була найсерйозніша прогалина документа. Повний розбір: [competitive-snapshot](research/competitive-snapshot.md).
...
| Конкурент | Сильні сторони | Де програє FlowIt |
|---|---|---|
...
1. **Стан не версіонується разом із кодом.** Не можна зробити `git checkout` на старий коміт і побачити, який тоді був спринт. У FlowIt — можна. Це унікальна властивість, якої немає в жодного конкурента.
...
- Конвенція «конфігурація і контекст живуть у репо» вже прийнята індустрією — FlowIt поширює її на управління задачами.
...
FlowIt — CLI-інструмент, який встановлюється в проєкт (`npx flowit init`) і створює директорію `.flowit/` у репозиторії. Усередині — весь стан проєкту: задачі, епіки, user stories, баги, спринти, беклог і документація.
...
| Фіча | Опис |
|---|---|
| `flowit init` | Створює `.flowit/` зі структурою та самоописовим README для агентів |
...
- **Читання — прямо з файлів.** Структура `.flowit/` самоописова, всередині лежить `README.md`, що пояснює агенту формат. Агент читає беклог звичайним Read/Grep, без жодної інтеграції. Працює з будь-яким агентом уже сьогодні.
- **Запис — тільки через CLI.** Агент викликає `flowit task move FLOW-12 in-progress` через Bash. CLI валідує й гарантує цілісність журналу. Пряме редагування файлів агентом не підтримується і має бути явно заборонене в README.
...
🔵 **Open Question:** як саме агент дізнається про існування CLI? Варіанти: генерувати секцію в `AGENTS.md`/`CLAUDE.md` під час `flowit init`; постачати Claude Code skill; покластися на README всередині `.flowit/`. Рекомендація — перше, як найбільш універсальне.
...
**Weekly Active Repositories** — кількість унікальних репозиторіїв, у яких за тиждень зафіксовано хоча б одну подію FlowIt.
...
🔵 **Open Question:** як це виміряти в OSS-інструменті без телеметрії? Варіанти: opt-in анонімна телеметрія (ризик для довіри спільноти), публічні репо через GitHub code search по `.flowit/`, або опитування. Рекомендація — code search як базовий сигнал плюс явно opt-in телеметрія.
...
Остання метрика — найважливіша для валідації стратегії. Якщо агенти не використовують FlowIt, ми побудували ще один task-менеджер.

### 6.3. Guardrail Metrics

| Guardrail | Поріг | Чому |
|---|---|---|
| Мердж-конфлікти в `.flowit/` | Близько до нуля | Перший же конфлікт на борді = втрата довіри й видалення інструменту |
| Час старту CLI/TUI | < 200 мс на 1000 задач | ✅ **Досягнуто: 28 мс** холодний старт, 7 мс теплий — на 10k подій, не 1k |
| Розмір `.flowit/` на диску | < 5 МБ на 10k подій | ✅ **Досягнуто: 1.9 МБ** після компакції (39.1 МБ без неї) |
...
*Як розробник, я хочу поставити FlowIt у свій репозиторій однією командою, щоб почати вести задачі без налаштувань.*

- [ ] `npx flowit init` створює `.flowit/` із базовою структурою
- [ ] Генерується `config.yml` зі статусами за замовчуванням (Todo, In Progress, In Review, Done)
- [ ] Генерується `.flowit/README.md`, що описує формат для AI-агента
...
- [ ] `flowit task add "текст"` створює задачу з автоматичним ID (`FLOW-N`)
...
- [ ] `flowit board` відкриває інтерактивний TUI з колонками зі `config.yml`
...
- [ ] `flowit sprint create --name --start --end`
- [ ] `flowit sprint add FLOW-12 FLOW-13` переносить задачі з беклогу
- [ ] `flowit sprint close` фіксує закриття; незавершені задачі повертаються в беклог або переносяться
...
- [ ] `flowit context` віддає компактний зріз стану, придатний для вставки в промпт
- [ ] `.flowit/README.md` явно забороняє пряме редагування `events/`
...
| Що | Чому не зараз |
|---|---|
| **Web UI / `flowit serve`** | Рішення прийнято: пізніше. Але це блокує Secondary-персону (PM Поліна) — вона не стане активним користувачем без візуального дашборду. Отже, це перший кандидат на v0.2. |
...
- **v0.2:** `flowit serve` — локальний веб-дашборд для PM; епіки та ієрархія; time tracking
...
| **Розробники не хочуть коммітити стан задач** — шум у PR | Середня | Високий | Опція окремої гілки для `.flowit/`; налаштування `.gitattributes` для приховування з diff |
...
1. **Команди відчувають брак контексту для агентів як біль**, а не як дрібну незручність. Якщо ні — унікальна цінність зникає, і FlowIt стає ще одним task-менеджером у переповненій категорії.
