...
export function runTemplateSave(
  ...
  return {
    ok: true,
    exitCode: 0,
    message: `Template "${trimmed}" saved: ${Object.keys(kept).join(', ')}.\n  kadence task add "title" --template ${trimmed}`,
    data: { schema: 'kadence/v1', ok: true, template: { name: trimmed, fields: kept } },
  };
}
...
export function runTemplateList(cwd: string, env: NodeJS.ProcessEnv): CommandResult {
  ...
  if (state.templates.length === 0) {
    return {
      ...
      data: { schema: 'kadence/v1', ok: true, templates: [] },
    };
  }
  ...
  return {
    ...
    message: state.templates
      ...
      .join('\n'),
    data: { schema: 'kadence/v1', ok: true, templates: state.templates },
  };
}
