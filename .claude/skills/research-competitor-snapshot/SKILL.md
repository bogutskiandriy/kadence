---
name: research-competitor-snapshot
...
source:
  ...
  path: pm-skills/skills/competitive-research-snapshot
...
or `research-market-landscape` output in session — the skill builds on evidence already gathered rather
...
- **Governing protocol:** honors the [`research-autonomous-investigation`](../research-autonomous-investigation/SKILL.md)
...
- **Discipline mix:** OSINT (press, reviews, analyst coverage) + FININT (filings, earnings language)
  ...
  intelligence-collection-disciplines (скіл відкладено).
...
- **Snapshot as baseline.** The schema is a contract with future runs: `competitive-intel-watch` (скіл відкладено)
...
- **When NOT to use:** you need market *sizing* → [`research-tam-sam-som`](../research-tam-sam-som/SKILL.md);
  you need deep intel on *one* company's strategy and executives →
  company-research (скіл відкладено) / [`research-company-intel`](../research-company-intel/SKILL.md);
...
1. Competitive battle card (battle-card-builder (скіл відкладено))
...
FSM-software market) that consumes the `research-market-landscape` example and becomes the baseline the
`competitive-intel-watch` (скіл відкладено) example diffs against. [`examples/sample-industrial.md`](examples/sample-industrial.md)
...
- **Regenerating instead of diffing.** Re-running this skill weekly and re-reading the whole output
  is theater — that's what competitive-intel-watch (скіл відкладено) is for.

## References

- [`research-autonomous-investigation`](../research-autonomous-investigation/SKILL.md) (Workflow) — the governing protocol
- intelligence-collection-disciplines (скіл відкладено) (Component) — discipline sources and signal chains
- [`research-market-landscape`](../research-market-landscape/SKILL.md) (Workflow) — upstream: surfaces which players deserve this snapshot
- competitive-intel-watch (скіл відкладено) (Workflow) — downstream: diffs future runs against this baseline
- battle-card-builder (скіл відкладено) (Workflow) — downstream: turns the snapshot into a field-action card
- company-research (скіл відкладено), [`research-company-intel`](../research-company-intel/SKILL.md) — single-company deep dives
