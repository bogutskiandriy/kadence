...
source:
  ...
  path: addy-agent-skills/skills/spec-driven-development
...
- **Interfaces live at the boundary.** The map records that `billing` depends on `identity`; the contract between them belongs in the provider module's spec (see `eng-api-and-interface-design` for designing it).
...
> Follow `eng-planning-and-task-breakdown` for the dependency-graph mapping and vertical-slicing mechanics behind these steps; it is the canonical source. The bullets above are a lightweight summary; if they ever diverge, `eng-planning-and-task-breakdown` takes precedence.
>
> **Output convention:** Save the plan to `tasks/plan.md` and record the task list in the task list target defined by `eng-planning-and-task-breakdown` (default `tasks/todo.md`; projects may designate an external tracker instead). Create `tasks/` if it does not exist. Downstream commands (`/build`, etc.) expect these defaults.
...
> Follow `eng-planning-and-task-breakdown` for the full task-sizing and dependency-ordering mechanics; it is the canonical source. The template below is a lightweight inline form; if they ever diverge, `eng-planning-and-task-breakdown` takes precedence.
...
Execute tasks one at a time following `../eng-incremental-implementation/SKILL.md` (`eng-incremental-implementation`) and `../eng-test-driven-development/SKILL.md` (`eng-test-driven-development`). Use `../eng-context-engineering/SKILL.md` (`eng-context-engineering`) to load the right spec sections and source files at each step rather than flooding the agent with the entire spec.
