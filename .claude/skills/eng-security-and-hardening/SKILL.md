...
source:
  ...
  path: addy-agent-skills/skills/security-and-hardening
...
**Operating rules:**
- **Minimize and set a purpose.** Collect a field only against a stated use. "It might be useful later" is not a purpose — it's latent breach scope. Don't log PII into telemetry (the `eng-observability-and-instrumentation` skill makes the same point from the ops side).
...
When data crosses a trust boundary, validate it as untrusted (see Input Validation above); when a privacy incident exposes personal data, the breach-notification clock is part of the postmortem — follow the `eng-debugging-and-error-recovery` skill.
