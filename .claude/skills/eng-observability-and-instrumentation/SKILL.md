...
**NOT for:**
- Diagnosing a failure happening right now — use the `eng-debugging-and-error-recovery` skill (observability is what makes that skill fast next time)
- Profiling and optimizing measured slowness — use the `eng-performance-optimization` skill
- Launch-day monitoring checklists and rollback triggers — see the `eng-shipping-and-launch` skill; this skill covers the instrumentation that feeds them
...
**Never log secrets, tokens, passwords, or full PII.** This is a hard rule from the `eng-security-and-hardening` skill — telemetry pipelines are a classic data-leak path. Allowlist fields; don't log whole request bodies.
